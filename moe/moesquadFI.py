...
def compute_metric_scores(prediction, reference, question_id, answer_start):
    ...
    squad_metric = evaluate.load("squad", experiment_id=f"squad_{time.time()}")
    
    # Prepare input as required by SQuAD evaluation, using the original question ID
    prediction_dict = {
        "id": question_id,
...
def main():
    ...
    num_trials = args.num_trials  # Only one bit flip trial per sample to keep total experiments manageable

    print(f"PyTorch version: {torch.__version__}")
    print(f"CUDA available: {torch.cuda.is_available()}")

    # Create output directory if it does not exist
    output_dir = "moesquadFI"
    ...
    # Reset random seed before bit flip experiments
    bit_flip_seed = 197  # Use a different random seed
    random.seed(bit_flip_seed)
    np.random.seed(bit_flip_seed)
    torch.manual_seed(bit_flip_seed)
    torch.cuda.manual_seed(bit_flip_seed)
    torch.cuda.manual_seed_all(bit_flip_seed)
    print(f"Random seed reset to {bit_flip_seed} to change fault injection positions")
    
    # Perform bit flip experiments - one trial per sample, as we have 100 samples
    ...
    for trial in range(num_trials):
        ...
        # Weighted random selection of module
        ...
        # Generate answers with fault-injected weights
