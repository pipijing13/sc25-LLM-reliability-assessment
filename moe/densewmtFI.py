...
def generate(id, dataset, tokenizer, model, max_length=100):
    ...
    source = dataset["translation"][id]["de"]  # Get source language text
...
def compute_bleu_score(prediction, reference):
    ...
    if not prediction or not reference:
        return 0.0
    bleu = evaluate.load("bleu", no_cache=True)
...
def compute_comet_score(prediction, reference, source):
    ...
    if not prediction or not reference:
        return 0.0
    comet = evaluate.load("comet", no_cache=True)
...
def compute_chrf_score(prediction, reference, word_order=2):
    ...
    if not prediction or not reference:
        return 0.0
    chrf = evaluate.load("chrf", no_cache=True)
...
def main():
    ...
    for idx in range(len(dataset)):
        ...
        # Write baseline translation to baseline file
        ...
        # Store baseline translation for later comparison
    ...
    # Close baseline translation file
